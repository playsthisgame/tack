...
Assemble a system prompt from the environment once at session start and inject it into every model call in the session, giving the model awareness of its working directory, project structure, and git context without requiring the user to describe them.
