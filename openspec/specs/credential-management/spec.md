...
Resolve provider API keys from layered sources (environment then secrets store) and surface a prompt for missing keys within the interface, keeping secrets separate from scoring configuration and never exposing key values in output or errors.
