...
The system SHALL present an interactive terminal interface with a visually bordered prompt
input and a scrolling transcript of turns. Submitting a prompt SHALL append a turn to the
...
- **THEN** the prompt input area is enclosed in a visible border that separates it from the
  transcript
